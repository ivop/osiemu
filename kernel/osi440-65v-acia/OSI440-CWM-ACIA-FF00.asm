...
LFF6C  JSR $BF2D  ;BASIC screen printer
       ...
       CMP #$0D
       BNE LFF9A
       PHA        ;upon writing <CR> append 10 <NULs>
       TXA
       PHA
       LDX #$0A
       LDA #$00
...
LFF8E  PHA
       LDA #$01   ;turn on ACIA input
       STA $0203
       LDA #$00   ;turn off save flag
LFF96  STA $0205
LFF99  PLA
LFF9A  RTS
...
LFFAE  LDA $DF01  ;Test for keypress
       BMI LFFA8
       JMP $A633  ;check for CTRL-C
...
LFFBC  PLA
       ...
       PLA
       TAX
LFFC0  LDA $DF01  ;check for key press
       BMI LFFD2  ;none if msb set
       PHA
       LDA #$00
       STA $0203
LFFCB  LDA $DF01  ;wait for ASCII Keyboard to finish
       BPL LFFCB  ;loop until key up
       PLA
       RTS

LFFD2  LDA $0203  ;test LOAD flag
       BEQ SFFAB  ;branch if not set
       LDA $FC00
       LSR A
       BCC SFFAB  ;no serial data, return to loop
       JMP LFFB6  ;else jump back to read waiting ACIA character
